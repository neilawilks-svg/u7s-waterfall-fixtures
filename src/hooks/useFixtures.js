    ...
    try {
      ...
      for (const zone of zones) {
        for (let pitchSlot = 0; pitchSlot < 2; pitchSlot++) {
          ...
          const match = findBestMatch(available, playedMatchups, clubMatchupsPerTeam, teamFixtureCounts, numRounds, adjacency, {}, {}, 0);
          if (!match) break;
      ...
      for (const zone of zones) {
        for (const pitch of zone.pitches) {
          ...
          const match = findBestMatch(candidates, playedMatchups, clubMatchupsPerTeam, teamFixtureCounts, numRounds, adjacency, {}, {}, 0);
          if (!match) continue;
          newRoundFixtures.push({
            id: `fixture-${roundNum - 1}-${pitch}`,
            round: roundNum, pitch, time: roundTime,
